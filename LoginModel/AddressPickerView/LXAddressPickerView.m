...
@interface LXAddressPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@end
@implementation LXAddressPickerView
...
- (UIView *)titleBackgroundView{
    if (!_titleBackgroundView) {
        _titleBackgroundView = [[UIView alloc]initWithFrame:
                                CGRectMake(0, self.bounds.size.height - TITLEHEIGHT, SELFSIZE.width, TITLEHEIGHT)];
...
- (UIButton *)cancelBtn{
    if (!_cancelBtn) {
        _cancelBtn = [[UIButton alloc]initWithFrame:
                      CGRectMake(0, 0, self.bounds.size.width/2.0, TITLEHEIGHT)];
        [_cancelBtn setTitle:@"取消"
                    forState:UIControlStateNormal];
        [_cancelBtn setTitleColor:[UIColor lightGrayColor]
                         forState:UIControlStateNormal];
...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        _sureBtn = [[UIButton alloc]initWithFrame:
                    CGRectMake(SELFSIZE.width /2.0f, 0, self.bounds.size.width/2.0, TITLEHEIGHT)];
        [_sureBtn setTitle:@"完成"
                  forState:UIControlStateNormal];
        [_sureBtn setTitleColor:[UIColor lightGrayColor]
                       forState:UIControlStateNormal];
...
- (UIPickerView *)addressPickerView{
    if (!_addressPickerView) {
        _addressPickerView = [[UIPickerView alloc]initWithFrame:
                              CGRectMake(0, self.bounds.size.height - 250.0f, SELFSIZE.width, 200)];
...
- (void)loadTitle{
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 0.f, self.bounds.size.width, 1.5)];
    lineView.backgroundColor = [UIColor lightGrayColor];
    [self.titleBackgroundView addSubview:lineView];
    
    UIView *lineView2 = [[UIView alloc] initWithFrame:CGRectMake(self.bounds.size.width/2.0f, lineView.bounds.origin.y + lineView.bounds.size.height, 1, 50.0f)];
    lineView2.backgroundColor = [UIColor lightGrayColor];
    [self.titleBackgroundView addSubview:lineView2];
