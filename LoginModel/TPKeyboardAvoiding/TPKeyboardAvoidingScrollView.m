...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    if (_Delegate && [_Delegate respondsToSelector:@selector(keyBoardShouldBegingEditing:)]) {
        return [_Delegate keyBoardShouldBegingEditing:textField];
    }
    return YES;
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (_Delegate && [_Delegate respondsToSelector:@selector(textField:shouldReplaceString:)]) {
        return [_Delegate textField:textField shouldReplaceString:string];
    }
    return YES;
}
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (_Delegate && [_Delegate respondsToSelector:@selector(keyBoardReturn:)]) {
        [_Delegate keyBoardReturn:textField];
    }
